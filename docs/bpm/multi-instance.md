...
## # 1. 多人审批
## # 1. 会签（并行会签）
![会签配置](../images/img_ed5a346e.png) 实现原理：
...
![BpmUserTaskActivityBehavior](../images/BpmUserTaskActivityBehavior.png) 所以，先是 BpmParallelMultiInstanceBehavior 计算审批任务数量 + 审批人列表，然后 BpmUserTaskActivityBehavior 直接设置审批人。
...
![BpmSequentialMultiInstanceBehavior](../images/BpmSequentialMultiInstanceBehavior.png) ② 还是使用 BpmUserTaskActivityBehavior，逻辑是一模一样的。
...
[选择审批人、发起人自选](/bpm/assignee/) [流程发起、取消、重新发起](/bpm/process-instance/) 
←
[选择审批人、发起人自选](/bpm/assignee/) [流程发起、取消、重新发起](/bpm/process-instance/)→
