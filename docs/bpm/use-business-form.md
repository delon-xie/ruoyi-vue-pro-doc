...
![新建流程](../images/img_08a94623.png) 注意，流程标识需要填 `oa_leave`。因为在 BpmOALeaveServiceImpl 类中，发起流程的标识是 `oa_leave`。
#### # 1.1.2 表单设计
切换到「表单设计」选项，选择 “流程表单”，填写如下图所示：
...
![设计流程](../images/img_a5604548.png) 可以点击 [`oa_leave_bpmn.XML`](/img/%E5%B7%A5%E4%BD%9C%E6%B5%81%E6%89%8B%E5%86%8C/%E5%A4%8D%E6%9D%82%E4%B8%9A%E5%8A%A1%E6%8E%A5%E5%85%A5/oa_leave_bpmn.XML) 进行下载，然后点击「打开文件」按钮，进行导入。
② 点击图中的“部门领导审批”，规则类型为“发起人部门负责人”。如下图所示：
![设置审批人 - 领导审批](../images/img_dc7a63fc.png) ③ 点击图中的“HR 审批”，规则类型为“岗位”，指定岗位为“人力资源”。如下图所示：
![设置审批人 - HR 审批](../images/img_873b6bb8.png) 
#### # 1.1.4 发布流程
① 点击右上角的「保存」按钮，保存流程模型的信息。
② 点击“OA 请求”的「发布」按钮，把定义的流程模型部署出去。部署成功后，就可以发起该流程了。如下图所示：
![发布流程](../images/img_be2f5d93.png) 
### # 2.2 第二步：发起流程
登录账号 admin、密码 admin123 的用户，扮演【员工】的角色，进行流程的发起。
① 发起业务表单请假流程，两种路径：
访问 [工作流程 -> 审批中心 -> 发起流程] 菜单，选择“OA 请求”流程，会跳转到流程模型 `oa_leave` 配置的表单提交路由。
![路径一：我的流程 -> 发起流程](../images/img_29129ff1.png) 访问 [工作流程 -> OA 示例 -> 请假查询] 菜单，点击「发起请假」按钮。
![路径二：请假查询 -> 发起请假](../images/img_3245c3d8.png) 
...
[审批接入（流程表单）](/bpm/use-bpm-form/) [流程设计器（BPMN）](/bpm/model-designer-bpmn/) 
←
[审批接入（流程表单）](/bpm/use-bpm-form/) [流程设计器（BPMN）](/bpm/model-designer-bpmn/)→
