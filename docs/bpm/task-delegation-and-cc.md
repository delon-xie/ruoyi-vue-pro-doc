...
![转办](../images/img_74c00dd8.png) 对应后端的 BpmTaskController 提供的 `#transferTask(...)` 接口。如下图所示：
图片纠错：最新版本不区分 yudao-module-bpm-api 和 yudao-module-bpm-biz 子模块，代码直接合并到 yudao-module-bpm 模块的 src 目录下，更适合单体项目
![转办的实现](../images/img_95775356.png) 
...
![委派](../images/img_3de3f48a.png) 它对应两个流程：
- A 转给其 B 审批
- B 审批后，转给 A
① A 转给其 B 审批，对应后端的 BpmTaskController 提供的 `#delegateTask(...)` 接口。如下图所示：
图片纠错：最新版本不区分 yudao-module-bpm-api 和 yudao-module-bpm-biz 子模块，代码直接合并到 yudao-module-bpm 模块的 src 目录下，更适合单体项目
![委派的实现](../images/img_191c97f8.png) 
...
图片纠错：最新版本不区分 yudao-module-bpm-api 和 yudao-module-bpm-biz 子模块，代码直接合并到 yudao-module-bpm 模块的 src 目录下，更适合单体项目
![审批委派的实现](../images/img_3d257be3.png) 
...
![发起抄送](../images/img_64bf3655.png) 对应的后端代码，如下图所示：
图片纠错：最新版本不区分 yudao-module-bpm-api 和 yudao-module-bpm-biz 子模块，代码直接合并到 yudao-module-bpm 模块的 src 目录下，更适合单体项目
![发起抄送的实现](../images/img_6d271aab.png) 
### # 2.3 查看抄送
被抄送人，可以在 [工作流程 -> 审批中心 -> 抄送我的] 菜单，查看到抄送列表，对应界面如下：
![抄送我的](../images/img_21210aa1.png) 
.pageB img{width:80px!important;}
.wwads-horizontal .wwads-text, .wwads-content .wwads-text{line-height:1;}
[审批加签、减签](/bpm/sign/) [执行监听器、任务监听器](/bpm/listener/) 
←
[审批加签、减签](/bpm/sign/) [执行监听器、任务监听器](/bpm/listener/)→
