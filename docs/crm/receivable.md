...
回款模块，由 `yudao-module-crm` 后端模块的 `receivable` 包实现，已经和 [BPM 工作流](/bpm/) 打通审批。
...
![回款管理](../images/img_c2d60a8d.png) ① 参考 [《【合同】合同管理、合同提醒》](/crm/contract) 文档，创建一个合同。注意，必须审核通过，才能进行回款。
② 点击【新增】按钮，随便填写一些信息，点击「确认」按钮，即可新增一条回款。如下图所示：
![新增回款](../images/img_23ee9517.png) 这样，我们就完成了【合同】=>【回款】的整个回款链路。
② 点击【提交审批】按钮，可以提交审批。如下图所示：
![提交审批](../images/img_47bf1820.png) 友情提示：
回款对应的审批流程，需要自己在 [工作流程 -> 流程管理 -> 流程模型] 菜单，配置一个流程标识为 `crm-receivable-audit` 的流程模型。如下图所示：
![流程模型](../images/img_d560e93a.png) 如果不了解怎么配置的同学，可以学习下 [《BPM 工作流》](/bpm/) 的文档。
提交完成后，会自动创建一个工作流实例，并记录到 `process_instance_id` 字段。之后可点击【查看审批】按钮，查看审批详情。如下图所示：
![查看审批](../images/img_2675b9e4.png) 点击审批【通过】按钮，将回款审批通过，此时它会回调 CrmReceivableResultListener 监听器，更新回款的 `audit_status` 字段为审批通过。如下图所示：
![审批通过](../images/img_e3c38272.png) ③ 点击“回款编号”，进入回款详情页，可以查看回款的详细信息，如下图所示：
![回款详情](../images/img_3de8b8a6.png) 另外，也可以在【合同】的详情页，进行【回款】的创建。如下图所示：
![新增回款（合同）](../images/img_7a334160.png) 
## # 2. 回款计划
...
![待回款提醒](../images/img_c6130bb7.png) 
### # 2.2 管理后台
对应 [CRM 系统 -> 回款计划] 菜单，对应 `yudao-ui-admin-vue3` 项目的 `@/views/crm/receivable/plan` 目录。
![回款计划](../images/img_139b7430.png) ① 点击【新增】按钮，随便填写一些信息，点击「确认」按钮，即可新增一条回款计划。如下图所示：
![新增回款计划](../images/img_a1ee6ad9.png) ② 点击【创建回款】按钮，可以创建回款。如下图所示：
![创建回款（回款计划）](../images/img_6cf033a3.png) 这样，我们就完成了【合同】=>【回款计划】=>【回款】的整个回款链路。
③ 点击“期数”，进入回款计划详情页，可以查看回款计划的详细信息，如下图所示：
![回款计划详情](../images/img_91316936.png) 另外，也可以在【合同】的详情页，进行【回款计划】的创建。如下图所示：
![新增回款计划（合同）](../images/img_c1ddf200.png) 
.pageB img{width:80px!important;}
.wwads-horizontal .wwads-text, .wwads-content .wwads-text{line-height:1;}
[【合同】合同管理、合同提醒](/crm/contract/) [【产品】产品管理、产品分类](/crm/product/) 
←
[【合同】合同管理、合同提醒](/crm/contract/) [【产品】产品管理、产品分类](/crm/product/)→
