...
![通过分佣提现](../images/img_1bdf2406.png) 此时，商城后端会调用 PayTransferAPI 的 `createTransfer` 方法，创建微信转账订单。它的内部，实际调用的是 [《微信转账 —— 发起转账》 (opens new window)](https://pay.weixin.qq.com/doc/v3/merchant/4012716434)。
具体的实现，可见 AppBrokerageWithdrawController 的 `#createBrokerageWithdraw(...)` 方法对应的 HTTP 接口。
补充说明：
① 微信转账 API，需要去 [https://pay.weixin.qq.com/index.php/extend/mch_sec_ip (opens new window)](https://pay.weixin.qq.com/index.php/extend/mch_sec_ip) 配置 IP 白名单。
