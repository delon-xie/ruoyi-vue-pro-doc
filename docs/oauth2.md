...
![授权模式的选择](/images/img_0deb77c5.png) 问题一：什么场景下，使用客户端模式（Client Credentials）？
...
- [《QQ OAuth 2.0 开发指定 —— 开发攻略_Client-side》 (opens new window)](https://wiki.connect.qq.com/%e5%bc%80%e5%8f%91%e6%94%bb%e7%95%a5_client-side)
- [《百度 OAuth —— Implicit Grant 授权》 (opens new window)](https://pan.baidu.com/union/doc/6l0ryrjzv)
问题五：该项目中，使用了哪些授权模式？
![项目使用的授权模式](/images/img_fc884d16.png) 如上图所示，分成 **外部授权** 和 **内部登录** 两种方式。
...
最终实现的整体架构，如下图所示：
![整体架构](/images/img_8f2276a5.png) 详细的代码实现，我们在视频中进行讲解。
## # 如何实现 SSO 单点登录？
...
![整体流程](/images/img_151bf2a4.png) 具体的使用流程如下：
...
![授权码模式的客户端信息](/images/img_edca0797.png) 
...
![授权码模式的未登录首页](/images/img_70d4a37f.png) 疑问：为什么没有跳转到 SSO 单点登录页，而是跳转到 ruoyi-vue-pro 项目的登录页？
...
![同意授权](/images/img_a8738d97.png) **⑥ 第六步**，完成授权后，会跳转到接入方的 callback.html 回调页，并在 URL 上可以看到 code 授权码。
![授权码模式的回调跳转](/images/img_fa2cfc19.png) **⑦ 第七步**，点击「确认」按钮，接入方的前端会使用 code 授权码，向接入方的后端获取 accessToken 访问令牌。
...
![授权码模式的登录后首页](/images/img_f981d63e.png) 示例代码的具体实现，与详细的解析，可以观看如下视频：
...
![整体流程](/images/img_1ff79861.png) 具体的使用流程如下：
**① 第一步**，分别启动 `ruoyi-vue-pro` 项目的前端和后端。
**② 第二步**，访问 [系统管理 -> OAuth 2.0 -> 应用管理 (opens new window)](http://127.0.0.1:1024/system/oauth2/oauth2/application) 菜单，新增一个应用（客户端），信息如下图：
![密码模式的客户端信息](/images/img_11d75561.png) 
...
![密码模式的未登录首页](/images/img_918f2383.png) ![密码模式的未登录首页2](/images/img_27fa4d81.png) **⑤ 第五步**，点击「登录」按钮，调用 `ruoyi-vue-pro` 项目的后端，获取到 accessToken 访问令牌，完成登录操作。
![密码模式的发起登录.png](/images/img_8e6623a5.png) **⑥ 第六步**，登录完成后，跳转回自己的 index.html 首页，并进一步从 `ruoyi-vue-pro` 项目获取到该用户的昵称等个人信息。后续，你可以执行「修改昵称」、「刷新令牌」、「退出登录」等操作。
![密码模式的登录后首页](/images/img_22a2e9b3.png) 示例代码的具体实现，与详细的解析，可以观看如下视频：
- [07、基于密码模式，如何实现 SSO 单点登录？ (opens new window)](https://t.zsxq.com/06rrrzBAu)
## # OAuth 2.0 表结构
![表结构](/images/img_3dfe2f1c.png) 每个表的具体设计，与详细的解析，可以观看如下视频：
...
## # 666. 社区贡献相关
- [《Pull Request：实现生成 apiKey 接口，便于第三方应用直接在 token 里携带过来进行认证》 (opens new window)](https://github.com/YunaiV/ruoyi-vue-pro/pull/718)、[前端部分 (opens new window)](https://github.com/yudaocode/yudao-ui-admin-vue2/pull/7)
.pageB img{width:80px!important;}
.wwads-horizontal .wwads-text, .wwads-content .wwads-text{line-height:1;}
[三方登录](/social-user/) [SaaS 多租户【字段隔离】](/saas-tenant/) 
←
[三方登录](/social-user/) [SaaS 多租户【字段隔离】](/saas-tenant/)→
