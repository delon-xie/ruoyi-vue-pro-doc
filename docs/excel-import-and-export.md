...
![EasyExcel](/images/01.png) 
## # 1. Excel 导出
以 [系统管理 -> 岗位管理] 菜单为例子，讲解它 Excel 导出的实现。
![系统管理 -> 岗位管理](/images/02.png) 
### # 1.1 后端导入实现
...
![handleExport 方法](/images/07.png) 
## # 2. Excel 导入
以 [系统管理 -> 用户管理] 菜单为例子，讲解它 Excel 导出的实现。
![系统管理 -> 用户管理](/images/11.png) 
### # 2.1 后端导入实现
...
![导入 Excel 接口](/images/12.png) 将前端上传的 Excel 文件，读取成 UserImportExcelVO 列表。
...
![UserImportExcelVO 代码](/images/13.png) 对应使用的 Excel 导入文件如下：
![UserImportExcelVO 文件](/images/14.png) 
#### # 2.1.2 ExcelUtils 读取
...
![DictConvert 实现](/images/21.png) 实现的代码比较简单，自己看看就可以明白。
...
![DictConvert 使用示例](/images/22.png) 
## # 4. 更多 EasyExcel 注解
...
[文件存储（上传下载）](/file/) [操作日志、访问日志、异常日志](/system-log/) 
←
[文件存储（上传下载）](/file/) [操作日志、访问日志、异常日志](/system-log/)→
