...
![新增数据源](../images/img_6dae98d3.png) 然后，手动将如下表拷贝到 `ruoyi-vue-pro` 主库中的如下表，拷贝到 `ruoyi-vue-pro-tenant-a` 库中。如下图所示：
![拷贝表结构到数据源](../images/img_34c6720a.png) system_dept
system_login_log
system_notice
...
![新增租户](../images/img_52d82579.png) 此时，在 `ruoyi-vue-pro-tenant-a` 库中，可以查询到对应的租户管理员、角色等信息。如下图所示：
![查询用户](../images/img_ede4412b.png) ③ 退出系统，登录刚创建的租户。
![登录界面](../images/img_f1518487.png) 至此，我们已经完成了租户的创建。
补充说明：
...
![使用示例](../images/img_2704c9e7.png) 注意，里面不能嵌套有 Spring 自带的事务，就是上图中【黄圈】的 Service 方法不能使用 Spring `@Transactional` 注解，否则会导致数据源无法切换。
如果【黄圈】的 Service 自身还需要事务，那么可以使用 `@DSTransactional` 注解。
.pageB img{width:80px!important;}
.wwads-horizontal .wwads-text, .wwads-content .wwads-text{line-height:1;}
[SaaS 多租户【字段隔离】](/saas-tenant/) [WebSocket 实时通信](/websocket/) 
←
[SaaS 多租户【字段隔离】](/saas-tenant/) [WebSocket 实时通信](/websocket/)→
