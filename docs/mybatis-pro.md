...
![分页案例 01](/images/01.png) ① 创建 `AdminUserMapper.xml` 文件，编写两个 SQL 查询语句：
...
* 查询分页的列表
*/
List selectPage01List(@Param("reqVO") UserPageReqVO reqVO);
/**
* 查询分页的条数
*/
...
@Override
public PageResult getUserPage(UserPageReqVO reqVO) {
...
![分页案例 01 的效果](/images/02.png) 
### # 1.2 方案二：MyBatis Plus XML
这个是 MyBatis Plus 拓展的使用方式，可以简化只需要写一条 SQL，步骤如下：
![分页案例 02](/images/01.png) ① 创建 `AdminUserMapper.xml` 文件，只编写一个 SQL 查询语句：
SELECT * FROM system_users
AND username LIKE CONCAT('%',#{reqVO.username},'%')
AND create_time BETWEEN #{reqVO.createTime[0]}, #{reqVO.createTime[1]},
AND status = #{reqVO.status}
...
@Mapper
public interface AdminUserMapper extends BaseMapperX {
IPage selectPage02(IPage page, @Param("reqVO") UserPageReqVO reqVO);
}
...
IPage page = new Page<>(reqVO.getPageNo(), reqVO.getPageSize());
...
@Mapper
public interface AdminUserMapper extends BaseMapperX {
default List selectList2ByStatusAndDeptName(Integer status, String deptName) {
return selectJoinList(AdminUserDetailDO.class, new MPJLambdaWrapper() // 查询 List
...
@Mapper
public interface AdminUserMapper extends BaseMapperX {
default List selectListByStatusAndDeptName(Integer status, String deptName) {
return selectJoinList(AdminUserDetail2DO.class, new MPJLambdaWrapper()
...
[MyBatis 数据库](/mybatis/) [多数据源（读写分离）、事务](/dynamic-datasource/) 
←
[MyBatis 数据库](/mybatis/) [多数据源（读写分离）、事务](/dynamic-datasource/)→
