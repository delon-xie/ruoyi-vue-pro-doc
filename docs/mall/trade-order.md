...
![订单状态流转](../images/img_04fa7afe.png) `cancel_type` 字段：取消类型，标记 `status` 为“已取消”的原因，由 TradeOrderCancelTypeEnum 枚举，例如说：超时未支付、退款关闭、买家取消。
...
![订单流程](../images/img_05b3edae.png) ![去结算](../images/img_21db5274.png) 
### # 2.1 结算【买家】
① 前端对应 `yudao-mall-uniapp` 项目的 `pages/order/confirm.vue` 页面，如下图所示：
![订单结算页](../images/img_260c1140.png) ② 后端对应 AppTradeOrderController 的 `#settlementOrder(...)` 提供的“获得订单结算信息”接口，核心的价格计算通过 TradePriceCalculator 实现。如下图所示：
图片纠错：最新版本不区分 yudao-module-trade-api 和 yudao-module-trade-biz 子模块，代码直接合并到 yudao-module-mp 模块的 src 目录下，更适合单体项目
![TradePriceCalculator 类图](../images/img_154d8e48.png) TradePriceCalculator 只是一个接口，它有多个实现类，满足不同的价格计算组合：
...
图片纠错：最新版本不区分 yudao-module-trade-api 和 yudao-module-trade-biz 子模块，代码直接合并到 yudao-module-mp 模块的 src 目录下，更适合单体项目
![TradeOrderHandler 实现类](../images/img_8bebd38c.png) 
...
![订单支付页](../images/img_9645d851.png) 这块已经在 [《微信公众号支付接入》](/pay/wx-pub-pay-demo/)、[《微信小程序支付接入》](/pay/wx-lite-pay-demo/) 文档中介绍过了，这里就不再赘述。
...
![支付结果页](../images/img_88fcd154.png) ④ 点击「查看订单」按钮，进入我的订单列表，对应 `yudao-mall-uniapp` 项目的 `pages/order/list.vue` 页面。如下图所示：
![订单列表页](../images/img_b4efce56.png) ⑤ 点击某个订单，进入订单详情，对应 `yudao-mall-uniapp` 项目的 `pages/order/detail.vue` 页面。如下图所示：
![订单详情页](../images/img_b2de719e.png) 此时，需要等待卖家发货。
### # 2.4 发货【卖家】
① 卖家可以在订单列表进行发货，对应 [商城系统 -> 订单中心 -> 订单列表] 菜单，对应 `yudao-ui-admin-vue3` 项目的 `@/views/trade/order` 目录。如下图所示：
![订单列表](../images/img_d528ce72.png) 当然，也可以在订单详情页进行发货，点击「详情」按钮进入，对应 `yudao-ui-admin-vue3` 项目的 `@/views/trade/order/detail.vue` 文件。如下图所示：
![订单详情](../images/img_b3d384da.png) ② 卖家点击「发货」按钮，进入发货页面，对应 `yudao-ui-admin-vue3` 项目的 `@/views/trade/order/delivery.vue` 文件。如下图所示：
![发货弹窗](../images/img_e3a4e771.png) 这里，我们选择物流公司为“申通快递”，并输入物流单号为“777168349863987”（因为它有真实物流数据），点击「确定」按钮，完成发货，更新 `trade_order` 为已发货（待收货）。
订单状态：已发货（待收货）
...
[【交易】购物车](/mall/trade-cart/) [【交易】售后退款](/mall/trade-aftersale/) 
←
[【交易】购物车](/mall/trade-cart/) [【交易】售后退款](/mall/trade-aftersale/)→
