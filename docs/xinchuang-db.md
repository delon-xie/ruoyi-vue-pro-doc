...
![新建模式](/images/img_674a6728.png) 为什么名字是 RUOYI_VUE_PRO 呢？
...
![数据库依赖](/images/img_1b55ae87.png) ② 修改 `application-local.yaml` 配置文件，将数据库的 `url`、`username`、`password` 修改为 DM 数据库。例如说：
![数据库连接配置](/images/img_0dc2f93f.png) ③ 后续启动项目，就可以了。
...
- [x86_64 版本 (opens new window)](https://kingbase.oss-cn-beijing.aliyuncs.com/KESV8R3/V009R001C001B0025-%E5%AE%89%E8%A3%85%E5%8C%85-docker/x86_64/kdb_x86_64_V009R001C001B0025.tar) 【Windows 选择这个】
...
![连接大金数据库](/images/img_ee4aa009.png) ② 选择 `test` 数据库的 `public` 模式，右键选择“运行 SQL 文件”，选择项目中的 `sql/kingbasees/ruoyi-vue-pro.sql` 文件，之后点击 [开始] 按钮。
### # 2.3 如何启动项目？
...
![数据库依赖](/images/img_1b55ae87.png) ② 修改 `application-local.yaml` 配置文件，将数据库的 `url`、`username`、`password` 修改为 KingbaseES 数据库。例如说：
![数据库连接配置](/images/img_4837f131.png) ③ 后续启动项目，就可以了。
### # 2.4 常见问题？
#### # 2.4.1 Quartz 定时任务？
Quartz 定时任务，已经适配，直接参考 [《定时任务》](/job) 使用。
实现思路非常简单：
...
- [《球友基于 AI + 文档适配成功》 (opens new window)](https://t.zsxq.com/hMzBJ)
...
![数据库依赖](/images/img_1b55ae87.png) ② 修改 `application-local.yaml` 配置文件，将数据库的 `url`、`username`、`password` 修改为 openGauss 数据库。例如说：
![数据库连接配置](/images/img_f43366d4.png) ::: 疑问：为什么 jdbc 使用 postgres 数据库连接？
...
[表结构变更（版本升级）](/sql-update/) [如何去除 Redis 缓存](/remove-redis/) 
←
[表结构变更（版本升级）](/sql-update/) [如何去除 Redis 缓存](/remove-redis/)→
