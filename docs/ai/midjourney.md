...
![私有的密钥配置](../images/img_4fd514d9.png) 
## # 2. AI 绘图
后续使用 [《AI 绘图》](/ai/image/) 时，需要在 [AI 大模型 -> 控制台 -> 模型配置] 菜单，配置对应的图像模型。
...
- `notify-url`：Midjourney 生成图片后的回调地址，因为 Midjourney 生成图片是异步的，所以需要回调通知。这个 URL 可以分成两个小部分： `http://java.nat300.top`：你的服务器地址。如果你没外网，可以通过 [《内网穿透》](/natapp) 解决
...
[【模型接入】Stable Diffusion](/ai/stable-diffusion/) [【模型接入】Suno](/ai/suno/) 
←
[【模型接入】Stable Diffusion](/ai/stable-diffusion/) [【模型接入】Suno](/ai/suno/)→
