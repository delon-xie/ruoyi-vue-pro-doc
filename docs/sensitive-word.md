...
## # 1. 实现原理
敏感词采用 [前缀树 (opens new window)](https://zh.m.wikipedia.org/zh-hans/Trie) 算法，，核心代码见 [SimpleTrie (opens new window)](https://github.com/YunaiV/ruoyi-vue-pro/blob/master/yudao-module-system/src/main/java/cn/iocoder/yudao/module/system/util/collection/SimpleTrie.java) 类。
## # 2. 使用教程
对应的管理后台，可以在 [系统管理 -> 敏感词] 菜单，进行敏感词的管理。如下图所示：
![敏感词管理](/images/img_4ef7a772.png) 
...
- 后端实现：[SensitiveWordController (opens new window)](https://github.com/YunaiV/ruoyi-vue-pro/blob/master/yudao-module-system/src/main/java/cn/iocoder/yudao/module/system/controller/admin/sensitiveword/SensitiveWordController.java)
### # 2.1 添加敏感词
![添加敏感词](/images/img_099cb7ab.png) 
- 标签：用于敏感词分组，不同的场景会需要使用不同的敏感词，通过标签进行分组。
添加完敏感词后，刷新下界面。
### # 2.2 测试敏感词
① 输入检测文本为“你是白痴么？”，选择标签为“测试”，检测到有敏感词：
![测试敏感词](/images/img_f327ac4d.png) ② 选择标签为“蔬菜”，检测到米有敏感词：
![测试敏感词](/images/img_041de288.png) 
## # 3. 敏感词的使用
...
[数据脱敏、字段权限](/desensitize/) [地区 & IP 库](/area-and-ip/) 
←
[数据脱敏、字段权限](/desensitize/) [地区 & IP 库](/area-and-ip/)→
