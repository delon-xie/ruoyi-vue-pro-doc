...
![管理后台 - 库存调拨](../images/img_e79dc5d3.png) ① 点击「新增」按钮，随便填写一些信息，点击「确认」按钮，即可新增一条调拨单。
![新增库存调拨](../images/img_552c3acd.png) ② 点击该调拨单的「审批」按钮，审批通过该出库单，此时状态会变成“已审批”，同时会减少来源仓库的库存、增加目标仓库的库存、新增 `2n` 条库存变动记录。如下图所示：
![审批库存调拨](../images/img_5833d45f.png) ③ 点击该调拨单的「反审批」按钮，反审批该调拨单，此时状态会变成“未审批”，同时会增加来源仓库的库存、减少目标仓库的库存、新增 `2n` 条库存变动记录。如下图所示：
![反审批库存调拨](../images/img_c8d394be.png) 
## # 2. 库存盘点
...
![新增库存盘点](../images/img_f4ac3308.png) ② 点击该盘点单的「审批」按钮，审批通过该盘点单，此时状态会变成“已审批”，同时会修改对应的库存、新增库存明细。如下图所示：
![审批库存盘点](../images/img_5038cfe5.png) ③ 点击该盘点单的「反审批」按钮，反审批该盘点单，此时状态会变成“未审批”，同时会修改对应的库存、新增库存明细。如下图所示：
![反审批库存盘点](../images/img_733f3f03.png) 
.pageB img{width:80px!important;}
.wwads-horizontal .wwads-text, .wwads-content .wwads-text{line-height:1;}
[【库存】其它入库、其它出库](/erp/stock-in-out/) [【采购】采购订单、入库、退货](/erp/purchase/) 
←
[【库存】其它入库、其它出库](/erp/stock-in-out/) [【采购】采购订单、入库、退货](/erp/purchase/)→
