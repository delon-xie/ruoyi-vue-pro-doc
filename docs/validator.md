...
public CommonResult getDictData(@RequestParam("id") @NotNull(message = "编号不能为空") Long id) {}
}
// Service 示例，一般放在接口上
...
- `views/infra/job/logger/index.vue` 的 `beginTime` 或 `endTime` 参数 ![单个时间传参](/images/img_30a54441.png) `views/system/user/index.vue` 的 `createTime` 参数 ![多个时间传参](/images/img_78892dbe.png) ### # 5.2 Request Body 时间传参 Request Body 时间传参，指的是 `Post`、`PUT` 等请求，通过 JSON 格式。 ① 后端接收时间参数时，需要添加 SpringMVC 的 `@RequestBody` 注解，使用 LocalDateTime 属性进行接收。例如说： 图片纠错：最新版本不区分 yudao-module-system-api 和 yudao-module-system-biz 子模块，代码直接合并到 yudao-module-system 模块的 src 目录下，更适合单体项目 ![ 注解](/images/RequestBody-server.png) ② 前端传递时间参数时，需要时间格式为 Long 时间戳。例如说： `views/system/tenant/TenantForm.vue` 的 `expireTime` 参数
![ 时间参数](/images/RequestBody-client.png) 
### # 5.3 Response Body 时间响应
JSON 返回的时间，使用 LocalDateTime 定义属性，会被序列化为 Long 时间戳进行相应。
例如说 TenantRespVO 的 `createTime` 属性，效果如下图：
![Response Body 时间响应](/images/ResponseBody-client.png) 
### # 5.4 如何自定义 JSON 时间格式？
...
![YudaoJacksonAutoConfiguration](/images/YudaoJacksonAutoConfiguration.png) 
#### # 5.4.1 全局配置时间格式
...
![全局配置时间格式](/images/img_ce832418.png) 
#### # 5.4.2 局部配置时间格式
...
[异常处理（错误码）](/exception/) [分页实现](/page-feature/) 
←
[异常处理（错误码）](/exception/) [分页实现](/page-feature/)→
