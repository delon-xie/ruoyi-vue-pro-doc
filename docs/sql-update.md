...
![Navicat](/images/Navicat.png) 友情提示：
...
![结构同步的数据库](/images/img_84d61536.png) ① 点击 Navicat 的 [工具 -> 结构同步] 菜单，配置源数据库是 `2.1.0`，目标数据库是 `2.0.0`，因为是将“新”应到“老”数据库。如下图所示：
![结构同步的新老数据库](/images/img_01cd4044.png) ② 点击左下角的 [选项] 按钮，取消掉“比较自动递增值”，因为我们不希望自增值也同步。如下图所示：
![结构同步的选项](/images/img_0e115700.png) ③ 点击右下角的 [下一步] 按钮，可以看到每个表的表更情况（新增、修改、删除）。如下图所示：
![结构同步的表更情况](/images/img_7df61e33.png) 此时，如果你只想看部分表的变更，可以点击“源对象”下的选项，取消掉不需要同步的表。
④ 继续点击右下角的 [下一步] 按钮，可以看到具体的 SQL 变更语句。如下图所示：
![结构同步的 SQL 变更语句](/images/img_42d51638.png) 注意，不要点击有效的 [开始] 按钮，而是将 SQL 语句复制出来，二次检查后，选择你需要的部分，手动执行变更！
...
![数据同步](/images/img_58d38bef.png) 系统绝大多数表的数据变更不用关注，只需要关注偏 **全局配置** 的表，例如说：
...
[删除功能（以租户为例）](/delete-code/) [国产信创数据库（DM 达梦、大金、OpenGauss）](/xinchuang-db/) 
←
[删除功能（以租户为例）](/delete-code/) [国产信创数据库（DM 达梦、大金、OpenGauss）](/xinchuang-db/)→
